...
import {getUser, getCards, updateAvatar, addCard, updateProfile} from './api.js'; 
...
// Утилитарная функция для управления состоянием загрузки кнопок
...
function handleEditFormSubmit(evt) { 
    ...
    // Сохраняем оригинальный текст кнопки при первом использовании
    if (!saveProfileButton.dataset.originalText) {
      saveProfileButton.dataset.originalText = saveProfileButton.textContent;
    }
    ...
    updateProfile(nameInput.value, jobInput.value)
      ...
      .finally(() => {
        renderLoading(saveProfileButton, false);
      });
} 
...
function handleAddFormSubmit(evt) { 
    ...
    // Сохраняем оригинальный текст кнопки при первом использовании
    if (!addCardButton.dataset.originalText) {
      addCardButton.dataset.originalText = addCardButton.textContent;
    }
    
    renderLoading(addCardButton, true);
    ...
    addCard(placeInput.value, linkInput.value) 
      ...
      .finally(() => { 
        renderLoading(addCardButton, false);
      }); 
} 
...
avatarForm.addEventListener('submit', function(evt) { 
  ...
  // Сохраняем оригинальный текст кнопки при первом использовании
  if (!saveAvatarButton.dataset.originalText) {
    saveAvatarButton.dataset.originalText = saveAvatarButton.textContent;
  }
  ...
  updateAvatar(newAvatarUrl) 
    ...
    .finally(() => { 
      renderLoading(saveAvatarButton, false);
    }); 
});